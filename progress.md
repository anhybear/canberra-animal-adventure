...
- Investigated a reported post-deploy blank load. Root cause: the service worker used a fixed `canberra-animal-adventure-v1` cache and cache-first HTML, so iPads could keep an old `index.html` pointing at deleted Vite hashed JS assets. Patched `public/sw.js` to v2, network-first navigations, avoid caching failed responses, purge old caches, and claim clients. Added an app-side `controllerchange` reload so normal future updates refresh once without interrupting service worker activation.
